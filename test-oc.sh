...
if [ -n "$1" ]; then
  oc create secret generic docker-creds --from-literal=DOCKER_USER=$1 --from-literal=DOCKER_PASSWORD=$2
fi

# Fire up pod
oc apply -f buildah-poc.yaml
...
oc cp docker.tar buildah-poc:/upload -c init-upload

# Mark as 'done'
oc exec buildah-poc -c init-upload -- touch /upload/done
